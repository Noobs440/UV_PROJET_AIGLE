...
export class ProjectDetailComponent {
  ...
  ngOnInit(): void {
    ...
    this.collaborateurService.getCollaborateurs().subscribe(response => {
      this.collaborators = response;
    });
    this.superviseurService.getSuperviseurs().subscribe(response => {
  ...
  openDialog(formType: any) {
    if (formType === 'supervisor') {
      // Vérifie s'il y a déjà un superviseur
      if (this.supervisor.length > 0) {
        alert('Un superviseur existe déjà. Vous ne pouvez que le modifier.');
        return;
      }
      // Ouverture du dialogue pour ajouter un superviseur
      ...
      dialogRef.afterClosed().subscribe(result => {
        console.log('Ajout superviseur terminé');
      });

    } else if (formType === 'update-supervisor') {
      // Vérifie s'il n'y a pas encore de superviseur
      if (this.supervisor.length === 0) {
        alert('Aucun superviseur à modifier. Veuillez en ajouter un d\'abord.');
        return;
            }
      // Ouverture du dialogue pour modifier le superviseur
      const dialogRef = this.dialog.open(DocumentPopupComponent, {
        width: '400px',
        height: '550px',
        data: { formType: formType }
      ...
      dialogRef.afterClosed().subscribe(result => {
        console.log('Modification superviseur terminée');
      });

    } else {
      // Cas des autres formulaires (document, collaborateur...)
      const dialogeRef = this.dialog.open(DocumentPopupComponent, {
        width: '400px',
        height: '550px',
        data: { formType: formType }
      });
